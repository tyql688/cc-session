...
/// Daily counts of sessions whose indexed text matches `keyword`, for the
/// trends chart. FTS phrase match for >=3-codepoint keywords, literal LIKE
/// for shorter ones — the same split as interactive search.
    ...
    if let Some(fts_query) = build_fts_query(keyword) {
        let mut stmt = conn.prepare(
            "SELECT date(s.updated_at / 1000, 'unixepoch', 'localtime') AS day,
             ...
             FROM sessions_fts
             JOIN sessions s ON s.rowid = sessions_fts.rowid
             WHERE sessions_fts MATCH ?1 AND s.updated_at >= ?2
             GROUP BY day
             ORDER BY day",
        )?;
            ...
            "SELECT date(updated_at / 1000, 'unixepoch', 'localtime') AS day,
             ...
             WHERE updated_at >= ?2
               AND (title LIKE '%' || ?1 || '%' ESCAPE '\\'
                 OR content_text LIKE '%' || ?1 || '%' ESCAPE '\\'
                 OR project_name LIKE '%' || ?1 || '%' ESCAPE '\\')
             GROUP BY day
             ORDER BY day",
        )?;
