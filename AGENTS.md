...
- SessionView is given `key={session.id}` to force remount on preview replacement (prevents stale local state: filters, search, favorite)
...
All providers implement `SessionProvider` trait (`source_roots` / `scan_all` / `scan_source` / `load_messages` / `deletion_plan` / `restore_action` / `cleanup_on_permanent_delete`).
...
| Kimi Code   | `~/.kimi-code/sessions/wd_*/<session_dir>/agents/*/wire.jsonl` | JSONL |
| Cursor CLI  | `~/.cursor/projects/<key>/agent-transcripts/<id>/<id>.jsonl` (CLI) + `~/.cursor/acp-sessions/<id>/store.db` (ACP) | JSONL + SQLite |
...
- **Provider snapshots**: backend derives provider label/color/order/path info; frontend consumes via `providerSnapshots` store
...
- **Session timeline uses content-visibility rendering** (NOT a JS virtualizer — those absolutely-position rows and self-manage scrollTop, which defeats the browser's scroll anchoring and makes scroll-up jump on every height correction). Every loaded row is real DOM in normal flow; the browser skips paint/layout for off-screen rows (`content-visibility: auto` + a per-row `contain-intrinsic-size` estimate from `estimateEntryHeight`), and an IntersectionObserver **paint-ahead band** force-paints rows within ~1.5 screens (`.session-entry.paint-ahead`) so fast scroll never blanks. Real normal-flow rows let native scroll anchoring keep the read position stable; the **native scrollbar is hidden** (its thumb would lurch downward when an older page prepends above) and the minimap is the scroll indicator. Message loading stays backend-windowed (tail on open, older pages prepend near the top; the prepend compensates scrollTop by the scrollHeight growth to keep content glued). Consequences: anything needing "all rows" runs on entry DATA (`searchHaystack` / `buildMatchLocations`); in-session search paints CSS Highlight ranges (applied to every loaded row, painted lazily as rows reveal); the minimap active turn comes from a throttled scroll hit-test of the top-visible row (→ `topVisibleMessageIndex` → outline), not a virtualizer range; keyboard paging (PageUp/PageDown/Home/End) is handled at document level in SessionView because WebKit routes paging keys only to the scroll area under the last click.
